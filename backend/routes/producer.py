...
async def get_producer_status():
    ...
    try:
        sensor_status = {
            "running": state.sensor_producer.running if state.sensor_producer else False,
            "type": "sensor_data"
        }
        
        detection_status = {
            "running": state.detection_producer.running if state.detection_producer else False,
            "type": "detection_data"
        }
...
async def start_sensor_producer():
    """启动传感器数据生产者"""
    try:
        if not state.sensor_producer:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail="传感器生产者未初始化"
            )
            
        if state.sensor_producer.running:
        ...
        import threading
        thread = threading.Thread(target=state.sensor_producer.start)
        thread.daemon = True
        thread.start()
...
async def stop_sensor_producer():
    ...
    try:
        ...
        if not state.sensor_producer.running:
            return ProducerResponse(
                message="传感器生产者已停止",
                status="already_stopped"
            )
            
        state.sensor_producer.stop()
...
async def start_detection_producer():
    """启动检测数据生产者"""
    try:
        if not state.detection_producer:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail="检测生产者未初始化"
            )
            
        if state.detection_producer.running:
            return ProducerResponse(
                message="检测生产者已在运行中",
                status="already_running"
            )
        ...
        import threading
        thread = threading.Thread(target=state.detection_producer.start)
        thread.daemon = True
        thread.start()
...
async def stop_detection_producer():
    ...
    try:
        ...
        if not state.detection_producer.running:
            return ProducerResponse(
                message="检测生产者已停止",
                status="already_stopped"
            )
            
        state.detection_producer.stop()

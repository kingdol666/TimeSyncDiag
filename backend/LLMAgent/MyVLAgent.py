...
"""
    ...
    def _search_knowledge_base(self, state: MyVLState) -> MyVLState:
        """搜索知识库，获取相关的历史案例和标准参数"""
        if not self.use_rag:
            return {"knowledge_result": "RAG 已禁用", "messages": []}

        query = f"""
        薄膜加工工艺故障诊断：
        工艺侧分析：{state['process_result'][:self.rag_query_max_length]}
        质检侧分析：{state['detect_result'][:self.rag_query_max_length]}
    ...
    def _run_workflow_with_langgraph_streaming(self, initial_state: dict):
        ...
        for mode, payload in self.workflow.stream(initial_state, stream_mode=["messages", "updates"]):
            ...
            if mode == "messages":
                ...
                if chunk.content:
                    yield {"event": "stream_content", "node": node_name, "content": chunk.content}
            elif mode == "updates":

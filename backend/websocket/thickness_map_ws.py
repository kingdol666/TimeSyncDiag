...
class ThicknessMapWebSocketManager:
    def __init__(self):
        ...
        # 注册配置变更回调
        config.register_change_callback("websocket.update_interval", self._on_update_interval_changed)
        config.register_change_callback("websocket.heartbeat_interval", self._on_heartbeat_interval_changed)
    
    def _on_update_interval_changed(self, key_path: str, value: Any):
        """配置变更回调：调整广播间隔"""
        self.update_interval = value
        logger.info(f"WebSocket 广播间隔已调整为 {value} 秒")
    
    def _on_heartbeat_interval_changed(self, key_path: str, value: Any):
        """配置变更回调：调整心跳间隔"""
        self.heartbeat_interval = value
        logger.info(f"WebSocket 心跳间隔已调整为 {value} 秒")

...
class ThicknessMapPipeline:
    ...
    def __init__(self):
        ...
        """
        # 当前执行间隔（秒），从配置读取
        self.interval = config.thickness_map.pipeline_interval
        ...
        # 注册配置变更回调：动态调整执行间隔
        config.register_change_callback("thickness_map.pipeline_interval", self._on_interval_changed)
    
    def _on_interval_changed(self, key_path: str, value: Any):
        """配置变更回调：调整执行间隔"""
        if key_path == "thickness_map.pipeline_interval":
            self.interval = value
            logger.info(f"膜厚温度云图处理间隔已调整为 {value} 秒")
    ...
    def initialize(self):
        ...
        try:
            ...
            self.db_connection = DatabaseConnection(self._build_database_url())
        ...
        try:
            ...
            if map_data:
                return {
                    ...
                    "min_value": map_data.min_thickness,
                    "max_value": map_data.max_thickness,
                    "avg_value": map_data.avg_thickness
                }
            else:
                return {
                    "success": False,

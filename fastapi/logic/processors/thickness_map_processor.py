...
class ThicknessMapProcessor:
    ...
    def _get_thickness_data_by_id(self) -> tuple[List[List[float]], datetime, datetime]:
        ...
        try:
            ...
            try:
                ...
                # 计算ID范围（最新ID向上40行）
                latest_id = latest_record.id
                min_id = max(1, latest_id - 39)  # 确保ID不小于1
    ...
    def _create_heatmap(self, thickness_data: List[List[float]], with_labels: bool = True, mask: Optional[np.ndarray] = None) -> bytes:
        ...
        try:
            ...
            if mask is not None and mask.shape == heatmap_data.shape:
                if heatmap_data.shape[0] > 4:
                    mask[-4:, :] = False
                    heatmap_data = np.where(mask, self.target_thickness, heatmap_data)
                else:
                    pass
    ...
    def _create_combined_heatmap(self, thickness_data: List[List[float]], 
        ...
        try:
            ...
            fig, axes = plt.subplots(9, 1, figsize=(16, 16),
            ...
            if is_abnormal:
                ...
                def run_agent_diagnosis():
                    try:
                        ...
                        # 检查qwen_vl_agent是否已初始化
                        if state.qwen_vl_agent is None:
                            logger.error("state.qwen_vl_agent 未初始化，无法进行诊断")
                            return
                        ...
                        result = state.qwen_vl_agent.run_workflow(
    ...
    def _reduce_mask(self, thickness_data: List[List[float]]):
        ...
        try:
            ...
            # 计算mask覆盖的行数（排除最后四行）
            if data_array.shape[0] > 4:
                mask_rows = np.any(self.mask_data[:-4, :], axis=1)
            else:
            ...
            # 计算向上移动的行数（每次移动10%的覆盖行数，至少移动1行）
            shift_rows = max(1, int(covered_row_count * 0.1))
            
            # 将mask向上移动（只移动除最后四行外的区域）
            if data_array.shape[0] > 4:
                movable_rows = data_array.shape[0] - 4
                if movable_rows > shift_rows:
                    self.mask_data[:-4-shift_rows, :] = self.mask_data[shift_rows:-4, :]
                    self.mask_data[-4-shift_rows:-4, :] = False
                else:
                    self.mask_data[:-4, :] = False

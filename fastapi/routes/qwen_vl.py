...
router = APIRouter(tags=["qwen-vl"])

class QwenVLRequest(BaseModel):
    """Qwen-VL 请求模型"""
    question: str

@router.post("/analyze-image", summary="分析两张图片并生成整合回答", description="上传两张图片，获取Qwen-VL模型的流式回答，包括检测分析、处理分析和整合结果", response_model=None)
async def analyze_image(
    ...
    Returns:
        流式返回的模型回答
    """
    # 检查QwenVL Agent是否已初始化
    if not state.qwen_vl_agent:
        raise HTTPException(status_code=500, detail="QwenVL Agent 未初始化")
    ...
    def generate_stream():
        ...
        try:
            # 尝试捕获 GeneratorExit
            for event in state.qwen_vl_agent.run_workflow(temp_detect_path, temp_process_path, thickness_map_uuid=thickness_map_uuid, stream=True):
            ...
            state.qwen_vl_agent.terminate_workflow()

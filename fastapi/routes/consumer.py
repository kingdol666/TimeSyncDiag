...
# 导入全局变量
from state import data_pipeline, detection_data_pipeline
...
async def get_consumer_status():
    ...
    try:
        consumer_status = {
            "running": data_pipeline.running if data_pipeline else False,
            "type": "data_pipeline"
        }
...
async def get_detection_consumer_status():
    ...
    try:
        consumer_status = {
            "running": detection_data_pipeline.is_running if detection_data_pipeline else False,
...
async def get_consumer_stats():
    ...
    try:
        # 获取传感器数据统计
        sensor_stats = {}
        if data_pipeline and hasattr(data_pipeline, 'stats'):
            sensor_stats = data_pipeline.stats
        
        # 获取检测数据统计
        detection_stats = {}
        if detection_data_pipeline and hasattr(detection_data_pipeline, 'stats'):
            detection_stats = detection_data_pipeline.stats
...
async def start_consumer():
    ...
    try:
        ...
        import threading
        thread = threading.Thread(target=data_pipeline.start)
        thread.daemon = True
        thread.start()
...
async def start_detection_consumer():
    """启动检测数据消费者"""
    try:
        if not detection_data_pipeline:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail="检测数据管道未初始化"
            )
            
        if detection_data_pipeline.is_running:
            return ConsumerResponse(
                message="检测消费者已在运行中",
                status="already_running"
            )
        ...
        import threading
        thread = threading.Thread(target=detection_data_pipeline.start)
        thread.daemon = True
        thread.start()
...
async def stop_consumer():
    ...
    try:
        ...
        if not data_pipeline.is_running:
            return ConsumerResponse(
                message="消费者未在运行",
...
async def stop_detection_consumer():
    ...
    try:
        ...
        if not detection_data_pipeline.is_running:
            return ConsumerResponse(
                message="检测消费者未在运行",
                status="already_stopped"
            )
